...
- **Per-pass guard (NOT a null ceiling):** a standalone single-id pass is admitted on `_is_scale_safe` plus a minimal non-null *population* floor (the pass must block more than a trivial handful of rows). It deliberately does **not** apply the compound-component `0.6` null ceiling: a per-id pass legitimately blocks only the rows that *have* that id, the static blocker filters null block keys so a high-null pass forms no giant null block (the FS-v2 lever-3 rationale), and coverage is restored by the OR across passes — not by any single pass. This is what lets `phone` (71% null) and `zip` (69% null) stay in the union, reproducing the canonical recall-lifting `[npi] | [email] | [phone] | [first_name,last_name] | [last_name,zip]`. (The strict `0.20 NULL_RATE_CEILING` on the *single-key* path is unchanged.)
